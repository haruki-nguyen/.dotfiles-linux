# ArcoLinux with AwesomeWM

First, tweak ArcoLinux with ArcoLinux Tweak Tool.
...
```bash
sudo pacman -S neovim tmux zip starship gitui inkscape gimp btop syncthing fd xclip libreoffice-fresh nodejs npm ripgrep noto-fonts-emoji foliate wine
```
...
- First, install a Nerd font on <https://www.nerdfonts.com/font-downloads> to `/usr/share/fonts/` (or install with the ArcoLinux Tweak Tool) and set it up for the terminal emulator. For example: Iosevka.
